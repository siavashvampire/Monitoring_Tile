...
class data_archive extends model implements modelInterFace {
	...
	public static  function  creatExportTable ( $variable ){
		$db = (model::db());
		$perfix = $db::$prefix ;
		model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS '.$perfix.'temp_table1 select SUM(arch1.counter) as counterAll ,arch1.Tile_Kind ,arch1.Sensor_id  ,arch1.phase ,arch1.unit from '.$perfix.'data_archive arch1 LEFT JOIN '.$perfix.'sensors sensor on ( arch1.Sensor_id = sensor.Sensor_id) WHERE sensor.Export = 1 and arch1.tileDegree = \'همه\' and '.$variable.' GROUP by arch1.Sensor_id,arch1.phase ,arch1.unit , arch1.Tile_Kind ORDER BY sensor.showSort ASC');
