...
class data_merge_Hour extends model implements modelInterFace {
...
;
 ');
	...
	public function mergeDB($startTime = null , $endTime = null ){
		$db = (model::db());
		$perfix = $db::$prefix ;
        $tempDBName = $perfix.'temp_merge_hour';
        model::queryUnprepared('DROP TABLE IF EXISTS '.$tempDBName.';');
        
		if ($startTime != null and $endTime != null){
            model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS '.$tempDBName.' select `Sensor_id`,DATE_FORMAT(MIN(Start_time), "%Y-%m-%d %H:00:00") AS Start_time,`JStart_time`,`AbsTime`,SUM(`counter`) as `counter`,`Shift_id`,`Shift_group_id`,`employers_id`,`Tile_Kind`,`Motor_Speed`,`phase`,`unit`,`tileDegree` from '.$perfix.'data_merge_Hour WHERE (Start_time BETWEEN "' . $startTime . '" AND "' . $endTime . '")  AND `Shift_id` <> -1 GROUP BY `Sensor_id` , HOUR(`Start_time`), `Shift_id`,`Shift_group_id`,`employers_id`,`Tile_Kind`,`phase`,`unit`,`tileDegree` ;');
        }
        else{
            model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS '.$tempDBName.' select `Sensor_id`,DATE_FORMAT(MIN(Start_time), "%Y-%m-%d %H:00:00") AS Start_time,`JStart_time`,`AbsTime`,SUM(`counter`) as `counter`,`Shift_id`,`Shift_group_id`,`employers_id`,`Tile_Kind`,`Motor_Speed`,`phase`,`unit`,`tileDegree` from '.$perfix.'data_merge_Hour WHERE `Shift_id` <> -1 GROUP BY YEAR(`Start_time`), MONTH(`Start_time`) , DAY(`Start_time`) , HOUR(`Start_time`) ,`Sensor_id` , `Shift_id`,`Shift_group_id`,`employers_id`,`Tile_Kind`,`phase`,`unit`,`tileDegree` ;');
           }
        model::queryUnprepared('INSERT INTO '.$tempDBName.' select `Sensor_id`,`Start_time`,`JStart_time`,`AbsTime`,SUM(`counter`) as `counter`,`Shift_id`,`Shift_group_id`,`employers_id`,`Tile_Kind`,`Motor_Speed`,`phase`,`unit`,`tileDegree` from '.$perfix.'data_merge_Hour WHERE `Shift_id` = -1  GROUP BY `Sensor_id` ,`phase`,`unit` ;');
        model::queryUnprepared('UPDATE  '.$tempDBName.' SET Start_time = NOW() WHERE  Shift_id = -1;');
        
        if ($startTime != null and $endTime != null){
            model::queryUnprepared('DELETE FROM '.$perfix.'data_merge_Hour WHERE (Start_time BETWEEN "' . $startTime . '" AND "' . $endTime . '");');
            model::queryUnprepared('DELETE FROM '.$perfix.'data_merge_Hour WHERE  Shift_id = -1;');
        }
        else{
            model::queryUnprepared('DELETE FROM '.$perfix.'data_merge_Hour ;');
        }
		model::queryUnprepared('INSERT INTO '.$perfix.'data_merge_Hour SELECT * from '.$tempDBName.';');
	}
    public function getHourData($sensors , $date ,$dayStart,$dayEnd,$movAvgFlag=false , $movAvg=3){
        $db = (model::db());
        $perfix = $db::$prefix ;
        ...
        model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS '.$tempDBName1.' SELECT SUM(data.counter) as counter , DATE_FORMAT(MIN(data.Start_time), "%Y-%m-%d %H:00:00") AS Start_time , data.Sensor_id as id, Hour(MIN(data.Start_time))+1 as Day , sensor.label as label FROM '.$perfix.'data_merge_hour AS data LEFT JOIN '.$perfix.'sensors sensor on ( data.Sensor_id = sensor.Sensor_id) WHERE `Start_time` BETWEEN "' . $dayStart . '" AND "' . $dayEnd . '" AND data.Sensor_id IN ' . $sensors . '  GROUP BY data.Sensor_id , DAY(data.`Start_time`),  hour(data.`Start_time`)  ORDER BY data.Start_time;');

        if (!$movAvgFlag){
            model::queryUnprepared('DELETE FROM '.$tempDBName1.' WHERE  `Start_time` NOT BETWEEN "'. $start .'" AND  "'. $end .'";');
