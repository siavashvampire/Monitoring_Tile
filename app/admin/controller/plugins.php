...
class plugins extends \controller {
		...
		if ( $appStatus == null ){
			...
			if ( file_exists($file_name) ) {
				...
				if ( isset($appData['db']) and !  is_null($appData['db'])) {
					$queries = $this->generateQueryCreatTable($appData['db']);
					if ( isset($appData['sqlInstall']) and  !  is_null($appData['sqlInstall'])  ){
                        $sqlQuery = $this->generateQueryFromSQl($appData['sqlInstall']);
                        $queries = array_merge($queries , $sqlQuery);
                    }
					model::transaction();
					try {
						$hasError = false ;
						foreach ( $queries as $tabelName => $query){
	...
	public function uninstallLocal($app = null){
		...
		if ( file_exists($file_name) ) {
			$appData = require_once $file_name;
			if ( isset($appData['db']) and !  is_null($appData['db'])) {
				$queries = $this->generateQueryDropTable($appData['db']);
                if ( isset($appData['sqlUnInstall']) and  !  is_null($appData['sqlUnInstall'])  ){
                    $sqlQuery = $this->generateQueryFromSQl($appData['sqlUnInstall']);
                    array_push($queries , $sqlQuery);
                }
				model::transaction();
				try {
					$hasError = false ;
...
//						file::removedir(payment_path.DIRECTORY_SEPARATOR.'app'.DIRECTORY_SEPARATOR.$app);
	...
	public function uninstallLocalPlugin($plugin = null){
		...
		if ( file_exists($file_name) ) {
			...
			Response::redirect(\app::getBaseAppLink('plugins/pLists/cantFindInfo/'.urlencode($file_name).'#app_'.$plugin));
			return false;
		}
	}
    ...
    private function generateQueryFromSQl($sql){
        $configDataBase = require payment_path. 'core'.DIRECTORY_SEPARATOR. 'config.php';
        if ( is_array($sql) ) {
            foreach ( $sql as  $sqlQuery) {
                $query[] = str_replace('{prefix}' ,$configDataBase['_dbTableStartWith'] , $sqlQuery);
            }
        } else {
            $query[] = str_replace('{prefix}' , $configDataBase['_dbTableStartWith'] , $sql);
        }
        return $query ;
    }
