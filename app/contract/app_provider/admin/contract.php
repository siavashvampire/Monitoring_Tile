...
//		$this->mold->set('access',$access);
    ...
    {
        $get = request::all('page=1,perEachPage=25,user,voteSendFrom,voteSendUntil,voteCompletedFrom,voteCompletedUntil,status=all,votesIds', null);
        $rules = [
            "page" => ["required|match:>0", rlang('page')],
            "perEachPage" => ["required|match:>0|match:<501", rlang('page')],
        ];
        ...
        if ($valid->isFail()) {
            ...
            if ($get['user'] != null) {
                ...
                $value[] = 0;
                $variable[] = 'uc.userId In ( ' . str_repeat('? ,', count($get['user'])) . ' ? )';
            }
        }
        ...
        model::join('user uc', ' co.userId = uc.userId ', "LEFT");
        ...
        $search = $model->search($value, ((count($variable) == 0) ? null : implode(' and ', $variable)), 'contracts_vote c', 'c.*,uc.userId as contactUserId,concat(uc.fname," ",uc.lname) as contactUserName ,v.voteName,u.fname,u.lname,ug.name, IF(c.fillOutDate is not null, "1", "0") as status', $sortWith, [$pagination['start'], $pagination['limit']]);
