...
class invoice extends innerController {
	...
	public static function generate($serviceId = null ,$baseData = null) {
		...
		$data = request::getFromArray($baseData, 'firstName,lastName,email,phone,price,description,customField,hookAction,returnTo,gatewayModule');
		unset($baseData);
		if (  $serviceId != null ) {
		...
		if ( isset($userSystem) ){
			$userModel = user::getUser($userSystem['value'],$userSystem['variable'] . ' = ?');
		}
		/* end getting user information */

		model::transaction();
		$error = false ;
		/* @var \app\invoice\model\invoice $invoiceModel */
		$invoiceModel = self::model(['invoice','invoice']) ;
		...
		if ( isset($userModel) ) {
			if ( $userModel->getUserId() != null )
				...
				$resultGenerateUser = user::generateUser(['fname'=>$data['firstName'],'lname'=>$data['lastName'],'email' => $data['email'],'phone'=>$data['phone'],'password'=>strings::generateRandomLowString(8),'groupId'=>1]);
		...
		$module = parent::callHooks('invoiceGateWays') ;
		if ( count($module) > 0 ) {
			if ( in_array($data['gatewayModule'] ,array_keys($module) ) )
				$moduleSelect = $data['gatewayModule'];
			else
		...
		if ( $invoiceId !== false ){
			...
			$itemsModel = self::model(['invoice','items']) ;
...
//			$user = user::getUserById($invoiceModel->getUserId());
//			if ( $user->getPhone() != null )
//				self::callHooks('sendSms' , ['phone' => $user->getPhone() , 'text' => rlang('newInvoiceSms').$invoiceModel->getPrice().rlang('newInvoiceSmsAfterPrice').$invoiceModel->getInvoiceId() .rlang('newInvoiceSmsAfterId')  , 'type' => 'newInvoice']);
//			if ( $user->getEmail() != null )
//				self::callHooks('sendEmail' , ['email' => $user->getEmail() , 'body' => rlang('newInvoiceSms').$invoiceModel->getPrice().rlang('newInvoiceSmsAfterPrice').$invoiceModel->getInvoiceId().rlang('newInvoiceSmsAfterId') , 'type' => 'newInvoice']);
//			unset($_SERVER['JsonOff']);
	...
	public static function startTransAction($invoice ){
		if ( is_int($invoice) ){
			...
			$invoice = self::model(['invoice','invoice'] , $invoice);
	...
	public static function checkTransAction($transaction ){
		if ( is_int($transaction) ){
			/* @var transactions $transaction */
			$transaction = self::model(['invoice','transactions'] , $transaction);
			if ( $transaction->getTransactionId() == null ){
				return self::jsonError( rlang('canNotFindTransaction'),500);
			}
		}
		/* @var \App\invoice\model\invoice $invoice */
		$invoice = self::model(['invoice','invoice'] , $transaction->getInvoiceId());
		if ( $invoice->getStatus() == 'paid')
			return self::jsonError( rlang('invoicePaidBefore'),500);
		...
		if ( $module[$moduleName]['payStatus'] ) {
			...
			if ( $transaction->upDateDataBase() ) {
				...
				if ( $invoice->upDateDataBase() ) {
					...
					if ( $invoice->getRequestAction() != null ) {
						$actions = explode(',',$invoice->getRequestAction());
						if ( isset($actions[1]) )
							self::callHooks($actions[0], [$actions[1],$invoice]);
						else
							self::callHooks($actions[0], [$invoice]);
					}
//					$_SERVER['JsonOff'] = false;
//					$user = user::getUserById($invoice->getUserId());
//					if ( $user->getPhone() != null )
//						self::callHooks('sendSms' , ['phone' => $user->getPhone() , 'text' => rlang('invoicePaidSms').$invoice->getInvoiceId() .rlang('invoicePaidSmsAfterId')  , 'type' => 'invoicePaid']);
//					if ( $user->getEmail() != null )
//						self::callHooks('sendEmail' , ['email' => $user->getEmail() , 'body' => rlang('invoicePaidSms').$invoice->getInvoiceId().rlang('invoicePaidSmsAfterId') , 'type' => 'invoicePaid']);
//					unset($_SERVER['JsonOff']);
					return self::json($invoice->returnAsArray()) ;
				} else {
