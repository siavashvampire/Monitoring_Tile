...
class landingPage extends controller {
	...
	public function insert(){
		...
		$files = file::get_files_by_pattern(__DIR__ . '/../../theme/'.$this->mold->getPath()['folder'] ,'*.content.mold.html');
		$template = [];
		for ( $i = 0 ; $i < count($files) ; $i++){
			$files[$i] = strings::deleteWordLastString($files[$i],'.content.mold.html');
			$files[$i] = strings::deleteWordFirstString($files[$i], __DIR__ . '/../../theme/'.$this->mold->getPath()['folder']);
