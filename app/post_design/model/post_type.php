...
class post_type extends model implements modelInterFace
{
    ...
    {
        if (count($id))
            return (parent::search([1], '? AND id IN (' . implode(',', $id) . ')', 'post_type post_type', 'id,Name'));
        else
            return true;
    }
    ...
    {
        if ($GroupId == $Admin){
            return (parent::search([1], '?', 'post_type type', 'id,Name'));
        }
        return (parent::search([$GroupId], 'type.evaluatorGroup = ?', 'post_type type', 'id,Name'));
