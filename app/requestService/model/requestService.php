...
class requestService extends model implements modelInterFace
{
    ...
    public function getMonthExportData($value, $variable, $showField = '*')
    {
        ...
        model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $perfix . 'request_temp_table2 SELECT (SELECT time_diff_all FROM ' . $perfix . 'request_temp_table1) as time_diff_all ,COUNT(data.requestId) as count ,SUM(TIMESTAMPDIFF(MINUTE,data.`Time_Start`,data.`Time_End`) * data.`HumanNumber`) as time_diff,data.phase,data.section,data.WorkerSection FROM ' . $perfix . 'request_temp_table0 data WHERE 1 GROUP by data.section,data.WorkerSection,data.phase;');
        model::queryUnprepared('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $perfix . 'request_temp_table3 SELECT sections.label as section,workerSections.label as workerSections,phases.label as phase, ROUND(data.time_diff / data.time_diff_all * 100 , 2 ) as percent, ROUND(data.time_diff / 60 , 2 ) as time_diff,data.count from ' . $perfix . 'request_temp_table2 data LEFT JOIN ' . $perfix . 'phases phases on phases.id = data.phase LEFT JOIN ' . $perfix . 'sections sections on sections.id = data.section LEFT JOIN ' . $perfix . 'sections workerSections on workerSections.id = data.WorkerSection;');
