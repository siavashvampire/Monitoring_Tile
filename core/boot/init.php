...
//		curl("https://www.paymentcms.ir/api/report/bug",$localData);
		if ( debugger ) {
			...
			httpErrorHandler::E500($error['file']);
		}
	}
	echo $html ;
	if ( $GLOBALS['timeStart'] != '' )
		echo '<!-- response in : ' . ( microtime_float() - microtime_float($GLOBALS['timeStart'] ) ) .' seconds -->';
	exit;
}
ob_start();
