...
class JDate {
	...
	public static function jgetdate($timestamp = '', $none = '', $timezone = 'Asia/Tehran', $tn = 'en') {
		...
		return array('seconds' => self::tr_num((int)self::tr_num($jdate[6]), $tn), 'minutes' => self::tr_num((int)self::tr_num($jdate[2]), $tn), 'hours' => $jdate[1], 'mday' => $jdate[3], 'wday' => $jdate[7], 'mon' => $jdate[5], 'year' => $jdate[8], 'yday' => $jdate[9], 'weekday' => $jdate[4], 'month' => $jdate[0], 0 => self::tr_num($ts, $tn));
	}
	...
	private static function jdate_words($array, $mod = '') {
		foreach ($array as $type => $num) {
			...
			switch ($type) {
				...
				case'rh':
					$key = array('یک شنبه', 'دو شنبه', 'سه شنبه', 'چهار شنبه', 'پنج شنبه', 'جمعه', 'شنبه');
					$array[$type] = $key[$num];
					break;
