...
class arrays {
	...
	private static function found_value_to_array_by_pattern($value,$array,$result,$key = null)
	{
		...
		$is_sum = false;
		$is_variable = false;
		if(!empty($key) && strings::strFirstHas($value,'+'))
		{
			$value = strings::deleteWordFirstString($value,'+');
			$is_sum = true;
		}
		if(strings::strFirstHas($value,'$'))
		{
			$value = strings::deleteWordFirstString($value,'$');
			$is_variable = true;
		}
	...
	{
		if (!is_array($pattern)) $pattern = self::multiexplode($delimiter, $pattern);
	...
	public static function parseParams($array, $keys, $default = null, $validation = null, $removeNull = false)
	{
		...
		if (is_array($keys)) {
			foreach ($keys as $key => $val) {
				...
				if (!is_numeric($key)) {
					$isHtml = strings::strhas($key, '!') ? true : false;
					...
					if (isset($array[$key]) && is_array($array[$key])) {
						...
						if (!empty($validation)) {
							if (!validate::checkOne($data[$key], $validation))
								$data[$key] = $val;
						}
					}
				} else {
					$isHtml = strings::strhas($val, '!') ? true : false;
			...
			foreach ($explodedKeys as $key) {
				...
				$isHtml = strings::strhas($cleanKey, '!') ? true : false;
				...
				if (isset($array[$cleanKey]) && is_array($array[$cleanKey])) {
					...
					$data[$cleanKey] = self::tr_num($value);
					if (!empty($validation)) {
						if (!validate::checkOne($data[$cleanKey], $validation))
	...
	public static function parseParam($array, $key, $default = null, $validation = null)
	{
		...
		if (!empty($validation)) {
			if (!validate::checkOne($value, $validation))
				$value = $default;
		}

		return self::tr_num($value);
	}
	...
	public static function  dataToStrArray($text, $replaces = array())
	{
