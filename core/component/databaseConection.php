...
//        $stmt->bind_param($typeDet, $searchVariable[0],$searchVariable[1]);
	 ...
	 * @throws Exception
	 */
	public function get($tableName, $numRows = null, $columns = '*' , $reset = true)
	{
		if (empty($columns)) {
			$columns = '*';
		}
	 ...
	 * @throws Exception
	 */
	public function getValue($tableName, $column, $limit = 1 , $reset = true )
	{
		$res = $this->ArrayBuilder()->get($tableName, $limit, "{$column} AS retval" , $reset);
		if (!$res) {
			return null;
		}
		if ($limit == 1) {
