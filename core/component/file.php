...
class file
{
    ...
    public static function convert_print_size($bytes, $round = 2)
    {
        $result = null;
        ...
        return (empty($result)) ? '0 B' : $result;
    }

    // get convert size : convert_size(1,"MB","KB") return 1024
    public static function convert_size($input, $type_input, $type_Convert, $print = false)
    {
        $input = intval($input);
    ...
    public static function get_files($directory, $no_file = array(), $exts = array(), $ext_action = "out")
    {
        ...
        foreach ($get_files as $get_file) {
            ...
            if (empty($no_file) || !in_array($get_file, $no_file)) {
                if ($check_ext) {
                    if (is_file($file)) {
                        $arr_files[] = $file;
                    }
