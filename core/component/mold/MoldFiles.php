...
class MoldFiles {
		...
		if (is_array( $ParametersCallWithMethod )  && count($ParametersCallWithMethod) > 1 ) {
			foreach ($ParametersCallWithMethod as $keyIndex => $key) {
				...
				if ( $tempIndex === false and end($FileWithPathArray) != null )
					continue ;
				$tempIndex = array_search( $this->generateFuncName($this->files[$tempIndex]['path']) , $this->functionCall );
				if ( $tempIndex === false )
					continue ;
				array_splice($this->functionCall, $tempIndex, 1);
			}
	...
	public function render(){
		...
		for( $i = 0 ; $i < count($this->files) ; $i++ ){
			...
			if ( ! ( isset($this->files[$i]['innerView']) and  $this->files[$i]['innerView'] ) )
				$this->functionCall[] = $return['function'] ;
		}
		if ( $this->cacheLifeTime === false )
			eval($evalString);
		ob_start();
		for( $i = 0 ; $i < count($this->functionCall) ; $i++ ){
			call_user_func_array($this->functionCall[$i] , [&$this->moldData] );
		}
		$render = ob_get_clean();
		return $render ;
	}
	...
	public function setMoldBlock($name , $data) {
		$this->moldData->setMoldBlock($name , $data);
	}


	public function getMoldBlock($name ) {
		return ( count($this->moldData->getMoldBlock($name))  > 0 ? $this->moldData->getMoldBlock($name) : array() ) ;
	}
...
//		$fileName = array_pop(explode(DIRECTORY_SEPARATOR,$File) );
		...
		if ( strings::strLastHas($app,':plugin') ) {
			...
			if ( $fileExtTree != strtolower( strings::deleteWordLastString($app,':plugin')) )
