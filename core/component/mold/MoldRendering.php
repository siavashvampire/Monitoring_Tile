...
class MoldRendering {
	...
	public static function emptyMap(){
		/* TODO: add best do for map empty */
		//self::$map = [];
	}
...
//			$this->moldShouldReplaceInTheEnd[  "'moldValueInner".$i."'" ] = $this->findName($matchesInner[$i][1]) ;
	...
	private function _setExplode($data){
		if ( !$this->engineStatus )
			return ;
		if (preg_match('/(name)( *)=/', $data['otherValues']) and preg_match('/(value)( *)=/', $data['otherValues'])  ) {
			$re = '/name(\ *)=\s*([\S]+)/';
			...
			preg_match_all($re, $data['otherValues'], $matches, PREG_SET_ORDER, 0);
			if (count($matches) != 1) die('error in set function1');
			$explode = end($matches[0]);
			$this->replace($data['find'], '$moldData->set(' . trim($name) . ' , explode(' . $explode.','.trim($value).') );');
		} else {
			$matches = explode(self::parameterSeparator , $data['otherValues'] );
			if (count($matches) != 2) die('error in set function');
			$this->replace($data['find'], '$moldData->set(' . trim($matches[0]) . ' , explode(' .$matches[1].','.trim($matches[2]).') );');
		}
	}
...
//			}
	...
	private function _callHook($data){
		if ( !$this->engineStatus )
			return ;
		if (preg_match('/(name)( *)=/', $data['otherValues']) ) {
...
//			if () {
				...
				if ( isset($set) ){
					$this->replace($data['find'], '$moldData->set(' . trim($set) . ' , $moldData->CallHook('.$class.','.$method.',['.$format.'] ) );');
				} else {
					if ( $this->runFromPhp ) {
						$this->replace($data['find'], ' $moldData->CallHook('.$class.','.$method.',['.$format.'] ) ');
					}else
						$this->replace($data['find'], 'echo $moldData->CallHook('.$class.','.$method.',['.$format.'] ) ; ');
				}
//			}
		} else {
			die('error in func function!(method is required)');
	...
	private function _break($data){
		if ( !$this->engineStatus )
			return ;
		if ( $this->runFromPhp ){
			$this->replace($data['find'],'break;' );
		} else {
			$this->replace($data['find'],'break; ' );
		}
	}
	...
	private function _unView($data){
		...
		$temp = explode(DIRECTORY_SEPARATOR,$savePatchFileWithName);
		$fileName = array_pop($temp);
		$this->replace($data['find'], '' , false);
		$this->string .= ' <?php } $this->unView(\''.$savePatchFileWithName.'\' ) ; { ?>' ;
	}
	...
	private function _map($data){
		...
		for ( $i = 0 ; $i < count($matches) ; $i++ ){
			...
			if ( isset( $this->map[$name] ))
				...
				$this->map[$name] = 0 ;
			$nameHash = md5($this->fileInfo['path'].$name.'_'.$this->map[$name]);
			$moldRendering = new MoldRendering($matches[$i][1],$this->fileInfo,$this->moldData,$this->version );
			$this->string .= ' <?php }  $this->setMoldBlock("'.$name.'" , "'.$nameHash.'" ); function moldBlock_'.$nameHash.'(&$moldData){ ?>'.$moldRendering->getResult() ;
		}
	...
	private function _url($data){
		...
		switch (trim(strtolower($data['otherValues']))) {
			...
			case 'themebase':
				...
				break;
			case 'themebasepath':
				$link = payment_path.'theme' .DIRECTORY_SEPARATOR. $this->fileInfo['folder'].DIRECTORY_SEPARATOR;
				$this->replace($data['find'],$link, false);
				return ;
				break;
			case 'patch':
				$listDirToFile = explode(DIRECTORY_SEPARATOR, $this->fileInfo['path']);
			...
			default :
				$linkAdd = '';
				if ( ! strings::strhas($data['otherValues'],'\'') and ! strings::strhas($data['otherValues'],'"') )
					$linkAdd = '\'';
				$link = '\app::getBaseAppLink(null, '.$linkAdd.$data['otherValues'].$linkAdd.')';
				break;
		}
		if ( $this->runFromPhp ){
			$this->replace($data['find'] , 'echo '.$link.' ; ' , false);
		} else
		$this->replace($data['find'],'echo '.$link.' ; '  , false,true);
	}
	...
	private function _call ( $data ){
		...
		$this->replace($data['find'],' foreach ( $moldData->getMoldBlock("'.$data['otherValues'].'") as $moldBlockName ) { if ( function_exists(\'moldBlock_\'.$moldBlockName) ) { call_user_func_array(\'moldBlock_\'.$moldBlockName ,[&$moldData] ); } } ');
	}
	...
	private function ___encrypt($value){
		if ( !$this->engineStatus )
			return ;
		return ' \paymentCms\component\security::encrypt('.$value.', \'base64\' , true ) ' ;
	}
	...
	private function ___convert_file_size($value,$round = 2 ){
		if ( !$this->engineStatus )
			return ;
		return ' \paymentCms\component\file::convert_print_size('.$value.','.$round.') ' ;
	}
	...
	private function ___implode($value, $search ){
		if ( !$this->engineStatus )
			return ;
		return 'implode('.$search.' ,'.$value.' )' ;
	}
	...
	private function ___html($value ){
		if ( !$this->engineStatus )
			return ;
		return 'html_entity_decode('.$value.' )' ;
	}
	private function ___persianNumber($value ){
		return 'str_replace(["1","2","3","4","5","6","7","8","9","0"],["۱","۲","۳","۴","۵","۶","۷","۸","۹","۰"],'.$value.' )' ;
	}
	...
	private function ___jDate($value,$format,$time_Zone = '"Asia/Tehran"' , $tr_num = '"en"'){
		if ( !$this->engineStatus )
			return ;
		return '( ( ctype_digit('.$value.') && strtotime(date(\'Y-m-d H:i:s\','.$value.')) === (int)'.$value.' ) ? \paymentCms\component\JDate::jdate('.$format.','.$value.',"",'.$time_Zone.','.$tr_num.') 
			:  
			( ctype_digit('.$value.') && strtotime(date(\'Y-m-d H:i:s\',('.$value.'/1000))) === (int)('.$value.'/1000) ) ? \paymentCms\component\JDate::jdate('.$format.',('.$value.'/1000),"",'.$time_Zone.','.$tr_num.')
			:
				\paymentCms\component\JDate::jdate('.$format.',strtotime(str_replace(\'/\', \'-\','.$value.')),"",'.$time_Zone.','.$tr_num.') ) ' ;
	}
}