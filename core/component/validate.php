...
class validate {
	...
	{
		if (!preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",self::getFirstData()))
			self::setError(rlang('ERROR_VALID_NAME'), self::$field_title);
	...
	private static function _floatInt()
	{
		if (! ( filter_var(self::getFirstData(), FILTER_VALIDATE_FLOAT) != false or filter_var(self::getFirstData(), FILTER_VALIDATE_INT) != false ) )
			self::setError(rlang('ERROR_INVALID_REQUEST'), self::$field_title);
	}
